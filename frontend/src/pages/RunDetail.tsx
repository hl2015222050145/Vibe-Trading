...
export function RunDetail() {
  ...
  const TABS: { id: Tab; label: string; icon: typeof BarChart3; hidden?: boolean }[] = [
    ...
    { id: "runCard", label: "运行卡", icon: FileCheck2, hidden: !hasRunCard },
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="grid gap-3 md:grid-cols-4">
        <RunCardStat label="Schema" value={card.schema_version || "未知"} />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <RunCardPanel title="回测摘要" icon={Database}>
          <KeyValueTable data={backtest} empty="没有回测摘要记录。" />
        </RunCardPanel>
        <RunCardPanel title="可复现性" icon={Fingerprint}>
          <KeyValueTable data={reproducibility} empty="没有可复现性哈希记录。" monospaceValues />
        </RunCardPanel>
      </div>

      <div className="grid gap-4 xl:grid-cols-2">
        <RunCardPanel title="指标" icon={BarChart3}>
          <KeyValueTable data={metrics} empty="没有标量指标记录。" />
        </RunCardPanel>
        <RunCardPanel title="验证" icon={ShieldCheck}>
          {card.validation ? (
            ...
            <p className="text-sm text-muted-foreground">没有验证数据记录。</p>
          )}
        </RunCardPanel>
      </div>
      ...
      <RunCardPanel title="产物校验和" icon={FileCheck2}>
        {artifacts.length > 0 ? (
          ...
          <p className="text-sm text-muted-foreground">没有产物校验和记录。</p>
        )}
      </RunCardPanel>
    </div>
  );
...
function ChartTab({ run }: { run: RunData }) {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {hasEquity && (
        <div>
          <h3 className="text-sm font-medium mb-1">权益曲线与回撤</h3>
...
function TradesTab({ run }: { run: RunData }) {
  ...
  if (trades.length === 0) return <div className="p-8 text-muted-foreground text-sm">没有交易记录。</div>;
  return (
    <div className="p-4">
      <table className="w-full text-sm">
        <thead>
...
function CodeTab({ code }: { code: Record<string, string> }) {
  ...
  if (files.length === 0) return <div className="p-8 text-muted-foreground text-sm">没有代码文件。</div>;
  return (
    <div className="flex flex-col h-full">
