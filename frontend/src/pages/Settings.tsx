  ...
  const localApiAccessSection = (
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      <div className="mb-4 space-y-1">
        ...
        <p className="text-sm text-muted-foreground">{"用于远程或私有 Web UI 部署。请在此浏览器中输入一次服务端 API key；本地 localhost 使用可留空。"}</p>
      </div>
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
        <label className="grid gap-2">
          <span className={labelClass}>{"服务端 API key"}</span>
          <input
            type="password"
            value={localApiKey}
            onChange={(event) => setLocalApiKeyState(event.target.value)}
            className={fieldClass}
            placeholder={"仅保存在此浏览器中。留空可清除。"}
      ...
      <p className="mt-2 text-xs text-muted-foreground">{"仅保存在此浏览器中。留空可清除。"}</p>
    </form>
  );
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">{"配置当前本地项目的模型凭证和市场数据源 token。"}</p>
        </div>
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">{"选择智能体使用的模型，并保存到项目本地的 agent/.env 文件。"}</p>
      </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{"用于中国 A 股、期货、基金和宏观数据。未设置时，项目会在可用场景回退到 AKShare。"}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
          ...
          <div className="rounded-md border bg-muted/20 p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              <span className={`rounded-full px-2 py-0.5 text-xs ${dataSettings.baostock_supported ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
                {dataSettings.baostock_supported ? "加载器可用" : "项目暂无加载器"}
              </span>
            </div>
