...
function ProgressRing({ current, total }: RingProps): JSX.Element {
  ...
  const dash = c * pct;
  return (
    <svg viewBox="0 0 24 24" className="h-3 w-3 text-primary shrink-0" aria-hidden="true">
  ...
  if (running.length === 1) {
    ...
    return (
      <div role="status" aria-live="polite" aria-atomic="true" className="min-w-0">
        <ToolRow entry={only} stepIndex={totalSoFar} totalSteps={running.length} eta={eta} />
      </div>
    );
  }

  return (
    <div role="status" aria-live="polite" aria-atomic="true" className={cn("min-w-0 space-y-1")}>
